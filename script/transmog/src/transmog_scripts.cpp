...
class transmog : public CreatureScript
{
public:
    transmog() : CreatureScript("transmog") { }
...
public:
    ...
    void OnLogin(Player* player)
    {
        ...
        if (result)
        {
            do
            {
                uint64 itemGUID = MAKE_NEW_GUID((*result)[0].GetValue<uint32>(), 0, HIGHGUID_ITEM);
                uint32 fakeEntry = (*result)[1].GetValue<uint32>();
...
public:
    ...
    void OnBeforeConfigLoad(bool reload) override
    {
        if (!reload) {
            ...
            sConfigMgr->LoadMore((conf_path + "/" + cfg_def_file).c_str());

            sConfigMgr->LoadMore((conf_path + "/" + cfg_file).c_str());
        }
    }
};
...
_module void FreeSC_transmog()
{
    ScriptRegistry<CreatureScript>::FreeScript("transmog");
    ScriptRegistry<GlobalScript>::FreeScript("global_transmog_script");
    ScriptRegistry<PlayerScript>::FreeScript("Player_Transmogrify");
    ScriptRegistry<WorldScript>::FreeScript("WS_Transmogrification");
}

_module void AddSC_transmog(bool reload) {
    new global_transmog_script();
    new transmog();
    new PS_Transmogrification();
    auto ws = new WS_Transmogrification();

	if (reload)
	{
		ws->OnBeforeConfigLoad(reload);
		ws->OnStartup();
		ws->OnAfterConfigLoad(reload);
	}
}
