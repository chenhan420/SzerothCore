...
class ScriptMgr
{
    ...
    public: /* Unloading */
		...
		void Unload();
		bool LoadScript(const char * name = "*.dll");
		bool FreeScript(const char * name);

		HMODULE GetScript(const char * name);
...
class ScriptRegistry
{
    public:
        ...
        // save removed pointer key
        static std::vector<std::pair<uint32, const char*>> RemovedPointerList;

        // After database load scripts
        static ScriptVector ALScripts;

        _static void AddScript(TScript* const script);
        _static bool FreeScript(const char * name);
        ...
        _static ScriptMapIterator GetScriptItrByName(const char * name);
...
template<class TScript> std::vector<std::pair<uint32, const char*>> ScriptRegistry<TScript>::RemovedPointerList;
