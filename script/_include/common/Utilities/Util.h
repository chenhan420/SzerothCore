...
#define STR_MAP(Ty) UNORDERED_MAP<const char*, Ty, strhash, strpred>

template <class Ty> std::vector<Ty> Str2Vec(std::string str, const char delimiter = ' ')
{
    std::istringstream iss(str);

	if (delimiter == ' ')
	{
		std::istream_iterator<Ty> is_itr(iss);
		std::vector<Ty> vec(is_itr, std::istream_iterator<Ty>());
	...
	{
		// do not use atoi to avoid data missing
		Ty _t = 0;
		int sign = 1;
		int offset = s.length();
...
template <class Ty> std::string Vec2Str(std::vector<Ty> vec, const char* split = " ")
{
	std::stringstream ss;
	std::copy(vec.begin(), vec.end(), ostream_iterator<Ty>(ss, split));
