...
:: If you have modified your vcxproj configurations in creating cmd, then you should update all existing script .vcxproj and .vcxproj.user files.

for /D %%i in (*) do (
if exist "%%i\%%i.vcxproj" (
del /f /s /q ".\%%i.vcxproj"
del /f /s /q ".\%%i.vcxproj.user"
call :CreateVcxproj %%i
call :CreateVcxprojUser %%i
echo %%i
)
)
